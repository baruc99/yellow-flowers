...
function setCanvasSize() {
  ...
  // Define el tamaño deseado para dispositivos móviles
  const mobileWidth = 400;
  const mobileHeight = 300;

  // Establece el ancho y alto del canvas según la resolución
  if (windowWidth <= mobileWidth) {
      canvas.width = mobileWidth;
      canvas.height = mobileHeight;
  } else {
      canvas.width = 800; // Ancho para pantallas no móviles
      canvas.height = 600; // Alto para pantallas no móviles
  }
}
